...
/*
	Problem 19: Output the Employee Name (Title + Surname), Title of Courtesy, Nationality, Age (Use Scalar Function), 
	EmployeeId, as well as the Years Hired using [HumanResources].[Employee] and [Sales].[Order] in NorthWind.
	The query should only display the employees that have been working at that company for the MAX Years Hire ordered by Age
	CROSS JOIN this gargatuan query with dbo.Nums and set N to represent the MAX years an Employee has worked
	at this location.  
*/
SELECT N.n AS SeniorityYear, [dbo].[getAge]([HireDate]) AS 'Years Working At Company' 
...
GROUP BY SO.EmployeeId
	...
	,HRE.HireDate
	,N.n
ORDER BY [dbo].[getAge]([BirthDate]) DESC
FOR JSON PATH
	,ROOT('Employee Seniority')
